...
9. In draft revision mode, write the next versioned draft beside the source draft, keep remaining/new `PA-*` / `PQ-*` workflow rows, keep analytics content, and reset `用户补充描述` to an empty placeholder.
