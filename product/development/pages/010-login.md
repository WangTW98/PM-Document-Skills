...
|---|---|---|---|---|
| 入口 | 启动页 / 强登录拦截 | 首次打开或 token 过期 | 无 | (页面假设 PA-001：App 采用强登录机制，不登录无法访问核心功能) |
...
| E-004 | 验证码输入框 | input | S-002 | 是 | 总是展示 | 请输入验证码 | 6位数字校验 | verify_code | (页面假设 PA-002：验证码统一为 6 位) |
...
| 埋点事件ID | 事件名称 | 事件类型 | 关联元素ID / ActionID | 触发时机 | 统计次数口径 | 去重规则 | 上报时报 | 关键属性 | 成功 / 失败归因 | 漏斗阶段 | 相关 PA/PQ |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
|---|---|---|---|---|---|---|---|
| PA-001 | App 采用强登录机制，不登录无法访问核心功能 | 1.2 页面入口与出口 | 访问逻辑 | 核心业务路径 | 高 | 待用户确认 | 确认为正式内容 |
| PA-002 | 验证码统一为 6 位 | 4. 页面元素清单 | 校验逻辑 | E-004, ACT-002 | 高 | 待用户确认 | 确认为正式内容 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要支持图形验证码防止刷接口？ | 6. 交互 Action | 安全性 | API-001 | 中 | 待用户确认 | 暂不添加 |
