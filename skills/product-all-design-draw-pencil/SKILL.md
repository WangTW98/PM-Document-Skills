---
name: product-all-design-draw-pencil
description: Orchestrate Pencil restoration from product/design one page at a time by maintaining a draw status file, selecting exactly one `*-pencil.json` artifact each round, and delegating execution to product-design-draw-pencil with MCP-stability and token-efficiency rules. Use when an AI agent needs to work through many Pencil JSON pages safely without batch rendering.
---

# Product All Design Draw Pencil

## Overview

This is an orchestration skill.

It does not replace `product-design-draw-pencil`.
It schedules repeated single-page Pencil restoration rounds until every eligible `*-pencil.json` artifact under `product/design/` has been processed.

The orchestration loop is:

1. discover candidate Pencil JSON files
2. initialize or refresh a status file
3. select exactly one unfinished page
4. delegate the actual draw to `product-design-draw-pencil`
5. persist the result
6. stop this round
7. continue on the next invocation

The skill is intentionally single-page-per-round to reduce MCP failure blast radius and token waste.
...
- `product/design/`
- the delegated single-page skill at `../product-design-draw-pencil/SKILL.md`
- the delegated contract reference at `../product-md-design/references/pencil-json-contract.md`

Optional inputs:

- an explicit target `.pen` document path
...
- `product/design/_design-draw-pencil-status.md`

Per invocation output rule:

- create or update the status file
- process exactly one eligible `*-pencil.json`
...
```md
# Design Draw Pencil Status

## Meta

- source_root: product/design
- target_pen_file: design/output/product-pages.pen
- total_pages: 18
- completed_pages: 4
- pending_pages: 12
- blocked_pages: 2
...
|---|---|---|---|---:|---|---|
| U-100 | 登录 | product/design/010-login/index-pencil.json | done | 1 | pages.pen / U-100 登录 | |
| U-210 | 待办列表 | product/design/030-todo/index-pencil.json | pending | 0 | pages.pen / U-210 待办列表 | |
| U-330 | 退款申请 | product/design/110-order/弹窗-退款申请-pencil.json | blocked | 2 | pages.pen / U-330 退款申请 | unresolved ref |
```
...
1. Resolve the source queue.
   - Scan `product/design/` for files ending in `-pencil.json`.
   - Ignore `-figma.json`, blocker files, status files, and unrelated artifacts.
   - Sort candidates deterministically by path.

2. Initialize or refresh the status file.
   - If `product/design/_design-draw-pencil-status.md` does not exist, create it from the discovered queue.
...
3. Minimize token load before delegation.
   ...
   - Read only the single selected `*-pencil.json` file for this round.
...
5. Delegate the draw.
   - Read `../product-design-draw-pencil/SKILL.md`.
   - Follow all of its single-page validation and restore rules.
   - Pass exactly one explicit `-pencil.json` source.
   - Reuse the same target `.pen` document when one has already been established, unless the user explicitly overrides it.

6. Classify failures conservatively.
   - Treat these as retryable transient failures:
     - MCP timeout
     - temporary tool unavailability
     - active Pencil document access failure
   ...
   - Treat these as non-retryable blocking failures:
     ...
     - unresolved `ref` semantics that would change visible meaning
     - impossible node restoration without visible regression
...
- Keep error digests short, such as:
  - `missing meta.page_id`
  - `unresolved ref`
  - `pencil document unavailable`
  - `mcp timeout`
- Do not paste raw stack traces or large tool output into the status file.
- Prefer one shared `.pen` document across many pages to reduce document creation churn.
...
- Never create a new `.pen` file every round unless the user asks for that behavior.
...
- discover the same set of Pencil JSON files
- derive the same next page from the status file
- delegate exactly one page to `product-design-draw-pencil`
