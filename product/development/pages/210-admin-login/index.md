...
- 当前输出文件：product/development/pages/210-admin-login/index.md
...
- 页面成功标准：合法账号的首次登录成功率及 MFA 验证的流畅度。
...
| S-003 | MFA 校验模态 | modal | 二步验证开启时 | E-006 | 弹出覆盖层 | API-002 | PA-001 |
...
| E-006 | MFA 验证码输入 | input | S-003 | 是 | 二步验证激活 | 请输入 6 位二次验证码 | 步进输入框 | mfa_code | PA-001 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| ACT-001 | E-005 | 提交初审 | 点击 | 表单非空 | 验证码校验 | 调用登录 API (API-001) | 成功或触发 MFA | 提示错误 | - | EVT-002 | 登录尝试 | - |  |
| ACT-002 | E-006 | 确认 MFA | 输入满 6 位 | - | - | 调用 MFA 校验 (API-002) | 进入后台 | 提示错误 | 跳转 M-200 | EVT-003 | 登录成功 | - | PA-001 |
...
| EVT-003 | 登录成功 | action | ACT-002 | 进入工作台 | 每次触发 1 次 | - | 实时 | user_role | - | 转化结束 |  |
...
|---|---|---|---|---|---|---|---|
| LoginResponse | need_mfa | boolean | 是 | API | false | - | 是否需要二次验证 |
| LoginResponse | token | string | 否 | API | - | - | MFA 通过后下发 |

### 8.2 接口清单

| API ID | 接口名称 | 方法 | 路径 | 触发 ActionID | 请求数据结构 | 返回数据结构 | 成功处理 | 失败处理 | 相关 PA/PQ |
|---|---|---|---|---|---|---|---|---|---|
| API-001 | 账号密码初核 | POST | /api/admin/auth/login | ACT-001 | {username, password, captcha} | {need_mfa, ticket} | 弹出 MFA 或登录 | 提示错误 |  |
| API-002 | MFA 二次核验 | POST | /api/admin/auth/mfa | ACT-002 | {ticket, mfa_code} | {token, user_info} | 写入 Token | 提示错误 | PA-001 |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 后台管理端强制要求 MFA 二步验证（如 Google Authenticator 或短信），作为等保要求 | 2. 页面结构 | 安全流程 | E-006, API-002 | 高 | 确认 | 不需要 |
| PA-002 | 后台不提供自助注册功能，所有账号由超级管理员在系统中手动创建 | 1.1 页面目标 | 功能范围 | - | 高 | 确认 | 更新到正式 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要支持“域账号登录 (SSO)”？当前 MVP 仅支持独立账号体系。 | 1.4 全局页面属性 | 登录方式 | - | 中 | 确认 | 不需要 |
