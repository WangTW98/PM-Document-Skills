...
mindmap
  root((导出预览页))
    导航与布局
      ...
      页面标题
    核心预览区
      简历实时预览 (HTML/WebView 渲染，支持缩放)
    模板选择区
      横向模板列表 (缩略图)
      VIP 标识与锁定状态
    操作区
      导出 PDF 按钮 (无水印，受限于导出次数)
      发送至邮箱/微信
    状态
      渲染中
      VIP 权限验证
...
|---|---|---|---|---|---|---|
| S-001 | 预览核心区 | content | 总是展示 | E-001 | 占据屏幕 60-70% 空间 | 本地渲染 |
...
|---|---|---|---|---|---|---|---|---|
| E-001 | 简历预览窗 | view | S-001 | 是 | 总是展示 | 渲染后的预览内容 | 阴影效果，支持双指缩放 | resume_data |
...
| E-003 | 渲染中 | 切换模板后加载 | local | 禁用，菊花转动 | 无 | 点击 | 正在渲染预览... | 渲染完成 |

## 6. 交互 Action 与执行效果

| ActionID | 触发元素ID | 交互名称 | 用户动作 | 前置条件 | 校验规则 | 执行动作 | 成功结果 | 失败结果 | 页面跳转 / 状态变化 | 埋点事件ID | 埋点事件名称 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| ACT-001 | E-002 | 切换模板 | 点击模板项 | 非锁定状态 | 无 | 更新本地预览 | 更新预览效果 | 提示渲染失败 | UI 更新 | EVT-002 | 预览页模板切换 |
...
|---|---|---|---|---|---|---|---|---|
| API-001 | 获取模板列表 | GET | /api/dict/templates | 页面加载 | 无 | {templates[]} | 渲染列表 | 错误提示 |
...
|---|---|---|---|---|---|---|---|
| M-001 | image | E-002 | API | WEBP/PNG | 渐进式加载 | 骨架屏 | 模板预览图 |
