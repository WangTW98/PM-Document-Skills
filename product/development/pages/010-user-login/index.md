...
- 当前输出文件：product/development/pages/010-user-login/index.md
- 页面级假设 / 待确认编号规则：
...
|---|---|---|---|---|---|
| platform_id | string | domain / route | derived from domain | Logo、品牌色、协议内容 | 基于当前域名识别所属平台（页面假设 PA-001） |
| login_method | enum | local state | "phone" | 登录表单展示内容 | 切换手机登录或微信登录 |
...
| S-002 | 登录卡片 | content | 总是显示 | E-003 至 E-012 | 居中垂直布局，圆角投影 | 本地状态 |  |
...
|---|---|---|---|---|---|---|---|---|---|
| E-001 | 品牌 Logo | image | S-001 | 是 | 总是显示 | 平台对应 Logo | 左对齐 | platform_id | PA-001 |
| E-002 | 服务目录链接 | nav | S-001 | 是 | 总是显示 | 服务目录 | 右对齐 | - |  |
| E-003 | 登录方式切换 | tab | S-002 | 是 | 总是显示 | 手机号登录 / 微信登录 | 居中等分 | login_method |  |
...
| E-007 | 登录按钮 | button | S-002 | 是 | login_method="phone" | 登录 / 注册 | 100% 宽，品牌实色 | - |  |
...
| ACT-002 | E-007 | 手机登录 | 点击 | 填写完整且勾选协议 | - | 调用登录接口 (API-002) | 存储 Token，跳转工作台 | 验证码错误提示 | 跳转 U-200 | EVT-003 | 登录按钮点击 | - |  |
...
| EVT-005 | 登录错误 | api_failure | API-002 | 接口返回错误 | 每次触发 1 次 | requestId | 实时 | error_code, error_msg | - | 异常 |  |
...
| login_method | enum | 是 | 页面状态 | phone / wechat | EVT-003 | 登录方式 |
| error_code | string | 否 | API 返回 | 10001 | EVT-005 | 错误码 |

### 7.3 关键统计次数说明

- 页面曝光次数：EVT-001，统计登录入口流量。
- 手机登录转化率：EVT-003 (result=success) / EVT-001。
...
|---|---|---|---|---|---|---|---|
| LoginState | phone_number | string | 是 | local | "" | E-004 |  |
| LoginState | verify_code | string | 是 | local | "" | E-005 |  |
...
|---|---|---|---|---|---|---|---|---|
| EDGE-001 | 验证码错误 | 接口返回 10002 | 红色 Toast | 输入框红色边框 | 重新输入 | 清空验证码位 | 验证码错误，请重新输入 |  |
...
| M-002 | image | E-001 | 远程 | PNG/SVG | 骨架屏 | 默认文字 Logo | 平台 Logo | PA-001 |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 登录注册页面的 Logo 和 协议内容根据当前访问域名动态匹配 | 1.4 全局页面属性 | 样式、内容 | E-001, E-011, E-012 | 高 | 确认 | 不需要，仅使用占位符替代 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要在登录页提供“账号密码登录”作为兜底？ | 1.1 页面目标 | 功能范围 | 元素清单、接口 | 低 | 确认 | 提供账号密码登录 |
