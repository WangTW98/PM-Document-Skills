...
- 来源 Sitemap 行：
  ...
  - 页面名称：进度字段配置
  - 页面类型：列表与表单
  - Sitemap 页面级MD文件：product/development/pages/240-progress-field-config.md
- 当前输出文件：product/development/pages/240-progress-field-config/index.md
...
| S-003 | 表单预览模态 | modal | 点击预览时 | E-005 | 弹出层，模拟 U-521 | - | PA-001 |
...
| E-005 | 实时预览按钮 | button | S-002 | 否 | 字段数 > 0 | 预览用户端表单 | 弱样式 | - | PA-001 |
...
| ACT-003 | E-005 | 开启预览 | 点击 | - | - | 弹出预览视图 | 展示 U-521 模拟效果 | - | - | - | - | 辅助功能不埋点 | PA-001 |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 预览功能仅提供 UI 渲染层面的模拟，不涉及真实的文件上传或数据提交接口模拟 | 2. 页面结构 | 预览体验 | E-005 | 高 | 确认 | 发布到正式 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要支持“字段联动”逻辑（如选 A 则展示 B 字段）？当前 MVP 仅支持平铺表单。 | 4. 页面元素清单 | 配置深度 | - | 中 | 确认 | 不需要 |
