...
|---|---|---|---|---|---|---|---|
| PA-001 | 转化率目标设为 80% | 1.1 页面目标 | 业务衡量指标 | 埋点分析 | 中 | 待用户确认 | 确认为正式内容 |
| PA-002 | 微信登录是必选项 | S-003 | 登录渠道 | E-005 | 高 | 待用户确认 | 确认为正式内容 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要支持一键登录 (如天翼/联通一键授权)？ | 4. 元素清单 | 登录方式 | E-001, E-002 | 中 | 待用户确认 | 暂不添加 |
| PQ-002 | 验证码有效期是 1 分钟还是 5 分钟？ | 8.2 接口清单 | 业务规则 | API-001, API-002 | 低 | 待用户确认 | 默认 5 分钟 |
