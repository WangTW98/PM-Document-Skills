...
> 输出路径：`product/release/mock/040-target-job.md`。本文档只描述页面展示内容与 mock 数据，不描述交互执行、埋点逻辑、接口调用或业务执行逻辑。
...
- 当前输出文件：`product/release/mock/040-target-job.md`
- 页面名称：岗位目标设置页
...
| 元素状态矩阵 | E-006 / 禁用 | 未填必填项 | 是 | 8. 状态文案与内容差异 | STATE-001 | |
| 交互 Action 与执行效果 | ACT-002 | 选择行业 | 是 | 6. 表单与选项 Mock 内容 | FORM-001 | |
...
| FORM-001 | E-003 | 行业 | select | 所属行业 | 请选择行业 | 互联网 / 移动互联网, 金融, 电子商务, 教育, 医疗健康, 其他 | 必填 | 请选择所属行业 | 动态 | 接口返回 |
...
|---|---|---|---|---|---|---|---|---|---|---|
| STATE-001 | E-006 | 禁用 | 禁用 | 目标岗位或行业为空时 | 生成 AI 分析报告 | 按钮变灰，不可点击 | 无 | 请填写目标岗位和行业 | 静态 | |
...
|---|---|---|---|
| 引导文案 | 请填写您的目标岗位，AI 将为您进行精准匹配分析 | | |
| 表单 Label | 目标岗位、所属行业、期望城市、职位描述 (选填) | | |
| 行业选项 | | [互联网, 金融, ...] | 接口返回 |
| 城市选项 | | [北京, 上海, ...] | 接口返回 |
| 提交按钮 | 生成 AI 分析报告 | | |
