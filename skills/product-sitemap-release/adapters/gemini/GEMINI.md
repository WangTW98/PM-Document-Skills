...
8. In draft revision mode, write the next versioned draft beside the source draft, keep remaining/new `A-*` / `Q-*` workflow rows, and reset `用户补充描述` to an empty placeholder.
