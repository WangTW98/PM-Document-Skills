...
1. Load orchestration context.
   ...
   - For processing, use the highest available versioned main draft in that bundle, such as `index-v3.md`; if none exists, use `index.md`.
...
3. Select one unfinished page bundle.
   ...
   - If the draft main file exists but lacks `## 7. 埋点事件统计设计`, lacks `埋点事件ID`, lacks any `EVT-*` event ID, or lacks `用户补充描述`, mark it `已阻塞` and record that `product-page-draft` must regenerate the draft bundle with analytics and user supplement support before release.
...
5. Write completion status.
   ...
   - Record status, draft bundle root, actual draft main path, release bundle root, generated overlay doc list, generation timestamp, source page ID, page name, and any blockers.
