...
|---|---|---|---|---|---|---|---|
| PA-001 | 预览采用高清图片流以保证移动端性能，导出时才生成正式 PDF | 4. 页面元素清单 | 性能方案 | E-001 | 高 | 确认 | 以HTML渲染，导出时才生成正式PDF |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 非 VIP 用户导出时是否强制增加水印？ | 1.3 角色与权限 | 业务策略 | E-003 | 高 | 确认 | 否，以导出次数限制，不需要水印 |
