...
> 输出路径：`product/release/mock/090-vip-subscription.md`。本文档只描述页面展示内容与 mock 数据，不描述交互执行、埋点逻辑、接口调用或业务执行逻辑。
...
- 当前输出文件：`product/release/mock/090-vip-subscription.md`
- 页面名称：会员订阅页
...
| 元素状态矩阵 | E-005 / 支付中 | 支付 Loading | 是 | 9. 状态文案与内容差异 | STATE-001 | |
| 交互 Action 与执行效果 | ACT-003 | 支付完成 | 是 | 10. Action 可见内容映射 | AM-001 | |
...
mindmap
  root((会员订阅页 Mock 内容))
    ...
    底部操作
      S-004 按钮: 立即支付 ¥19.9
      S-004 勾选: 同意“会员连续包月协议”及“隐私政策” (默认勾选)
```
...
| S-004 | S-004 | 底部操作区 | 触发支付流程 | 支付总金额显示、“立即支付”按钮及协议勾选框 | 动态 | 套餐关联 | 支付中状态 |
...
| E-006 | - | 协议勾选框 | checkbox | S-004 | 我已阅读并同意“会员连续包月协议”及“隐私政策” | 静态 | | 默认选中 | | | | |
...
| FORM-001 | E-006 | 协议勾选 | checkbox | 同意协议 | 已选中 | | | 请先同意会员服务协议 | 静态 | |
...
|---|---|---|---|---|---|---|---|---|---|---|
| STATE-001 | E-005 | 支付中 | 加载 | 调起支付 SDK 后 | 支付处理中... | 按钮禁用，文案改变 | 无 | 系统支付弹窗已弹出 | 静态 | |
...
|---|---|---|---|---|---|---|---|---|---|
| AM-001 | ACT-003 | SDK | 支付完成 | 回跳个人中心 | 支付成功 | 支付失败 | Toast: 恭喜，您已成为 VIP 会员 | 静态 | |
...
|---|---|---|---|
| 权益标题 | 无限次 AI 分析、高级模板库、专属导出 | | |
| 月度价格 | | ¥19.9 | 接口返回 |
| 年度价格 | | ¥168 | 接口返回 |
| 按钮文案 | 立即支付 | | |
| 协议文案 | 我已阅读并同意“会员连续包月协议”及“隐私政策” | | |
| 失败提示 | 支付已取消 / 支付遇到问题 | | |
