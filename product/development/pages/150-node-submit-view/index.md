...
|---|---|---|---|---|---|---|---|
| PA-001 | 节点的提交表单结构由服务模板配置下发，前端根据 schema 自动渲染字段与校验规则 | 1.4 全局页面属性 | 动态表单 | E-002, API-001 | 高 | 确认 | 更新到正式 |
| PA-002 | 文件上传需支持分片上传以应对大文件（如专利扫描件），且支持断点续传 | 6. 交互 Action | 文件上传 | ACT-001 | 中 | 确认 | 更新到正式 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要支持“OCR 识别”功能（如营业执照自动填表）？当前 MVP 假设为手动填写。 | 4. 页面元素清单 | 自动化 | E-002 | 低 | 确认 | 不需要 |

## 12. 用户补充描述

```text
1，当前页面除展示当前进行中的步骤外，还应以进度条形式展示各个节点的进度，包括已完成、进行中和未完成的节点。
2，除用户上传提交资料外，部分服务类型会出现让用户下载资料并通过手动填写（如盖章或签名）后，上传电子版提交审核的情况，此时进度应展示下载资料并填写相关内容，此步骤文件来源于管理端业务员填写表单（上传模版文件、填写说明文本表单）。
3，部分服务类型会出现用户填写表单后，需要上传文件，此时进度应展示上传文件，此步骤文件来源于用户上传。
```
