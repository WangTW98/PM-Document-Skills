...
| 来源章节 | `Sitemap 页面生成总表` |
| 上一版来源文件 | `product/development/layout/product-layout-draft-user-web.md` |
| 当前输出文件 | `product/development/layout/product-layout-draft-user-web-v2.md` |
| Layout Key | `user-web` |
| 适用 Surface | `用户端` |
| 适用端形态 | `desktop-web` |
...
| 页面层级模型 | Root 页面作为导航目的地，L2 列表/详情在壳内切换；案件详情下的节点资料提交流程使用独立新页面承载，以保留完整步骤上下文 | 来源：release 文档；已应用 LA-001 |
| 响应式 / 端形态策略 | 当前仅做 desktop-web，窄桌面以容器收缩、筛选折行、表格横向滚动适配 | 已确认 LA-002 |
...
|---|---|---|---|---|---|---|---|---|
| SF-001 | 用户端 PC Web（双域名同构） | 游客、客户用户 | top-nav | top-nav / content-header / main-scroll / floating-service-entry / toast-layer / legal-footer | 登录注册、用户工作台、服务目录、订单中心、我的服务、消息与个人中心 | nested-route / push | 登录注册、支付收银台、协议查看页、节点资料提交页 | 来源：release 文档；已应用 LA-001、LQ-001 |

## 4. 全局 Shell 区域规范

| Shell 区域ID | 区域名称 | 所属 Surface | 显示规则 | 内容槽位 | 尺寸 / 安全区 | 滚动关系 | 层级关系 | 状态变化 | 来源 / LA / LQ |
|---|---|---|---|---|---|---|---|---|---|
| SHELL-001 | Top Navigation | SF-001 | 未登录与登录后均显示，当前一级导航高亮；双域名仅替换品牌视觉，不显示平台标签切换 | logo / nav-items / message-entry / account-entry | 固定高 64px | fixed | 高于主内容、低于弹层 | default / sticky / compact | 已确认 LA-003；已应用 LQ-001 |
| SHELL-002 | Main Content Container | SF-001 | 所有业务页面显示 | page-title / filter-bar / body / side-summary | 最大宽 1280px | main-scroll | 位于顶栏下方 | loading / empty / error / permission | 已确认 LA-004 |
...
| SHELL-004 | Modal Layer | SF-001 | 交易确认、二次确认、轻量说明弹窗显示 | modal-header / body / fixed-actions | 中央弹层，宽度随内容自适应 | page-scroll-locked | 最高交互层 | open / dirty / submitting / success | 来源：release 文档 |
...
| 20 | U-200 | ROOT | L1 | 用户工作台 | product/development/pages/020-user-dashboard.md | SF-001 | top-nav | TPL-004 | top-nav:工作台 | nested-route | SHELL-001 / SHELL-002 / SHELL-003 | 宽屏摘要+入口卡片 | 登录后默认首页 | 已确认 LA-009 |
...
| 50 | U-300 | ROOT | L1 | 服务目录 | product/development/pages/050-service-catalog.md | SF-001 | top-nav | TPL-001 | top-nav:服务目录 | nested-route | SHELL-001 / SHELL-002 / SHELL-003 | 类目网格 | 双域名仅品牌差异 | 已应用 LQ-001 |
...
| 110 | U-420 | U-400 | L2 | 订单详情 | product/development/pages/110-order-detail.md | SF-001 | top-nav | TPL-002 | top-nav:订单 | push | SHELL-001 / SHELL-002 / SHELL-003 / SHELL-005 | 主副栏详情 | 客服为悬浮入口，不占正文 | 来源：release 文档；LA-008 |
...
| 150 | U-521 | U-520 | L3 | 节点资料提交视图 | product/development/pages/150-node-submit-view.md | SF-001 | top-nav | TPL-006 | hidden | push | SHELL-001 / SHELL-002 / SHELL-003 / SHELL-005 | 表单区单列，摘要区按需吸附 | 从案件详情进入独立页面，保留案件上下文入口 | 已应用 LA-001 |
...
|---|---|---|---|---|---|---|
| NAV-001 | 用户端 Root 页面 | 一级导航使用顶栏切换 `工作台 / 服务目录 / 订单 / 我的服务 / 个人中心`，双域名不显示平台标签切换 | 切换后保留登录态 | 顶栏高亮当前入口 | 游客仅可见服务目录与登录入口 | 来源：release 文档；已应用 LQ-001 |
| NAV-002 | 用户端详情/交易流 | 从列表进入详情、下单、支付采用 push 路由 | 返回回到来源页及滚动位置 | 顶栏保持父级入口高亮 | 未登录需先跳登录 | 已确认 LA-010 |
| NAV-003 | 用户端案件 L3 提交流程 | 节点资料提交使用独立页面承载，并通过 breadcrumb / 返回入口与案件详情保持关联 | 返回回到案件详情当前位置 | 顶栏保持 `我的服务` 高亮 | 无权限时显示 permission 容器 | 已应用 LA-001 |
...
| 客户用户 | 用户端全局 | 显示完整一级导航；退款按钮仅在符合条件订单显示；节点资料提交作为独立步骤页进入 | 无 | 来源：release 文档；已应用 LA-001 |
...
| product-page-mock-draft | 匹配的 product-layout-release*.md + release page | 补齐顶栏、页面标题、状态容器文案、悬浮客服入口与节点提交页的 breadcrumb / step 区 | Layout Key / 全局区域 / 导航标签 / 页面标题 / 状态容器 |
| product-page-design-release | 匹配的 product-layout-release*.md + release mock + design system | 生成顶栏壳、内容容器、模态层、固定状态带与滚动关系；U-521 使用独立页面模板，不再套用抽屉壳 | Layout Key / Shell 区域 / 页面模板 / 父子层级 / scroll/fixed 关系 |
...
| LA-006 | 用户端抽屉宽度统一为 520px。 | 4. 全局 Shell 区域规范 | Shell / 模板 | 历史草稿中的 SHELL-004、轻量确认抽屉 | 中 | 确认 | 保留为轻量弹层/抽屉尺寸约束，不再用于 U-521 |
| LA-007 | 顶栏下方状态横幅为统一承载支付超时、退款处理中等流程状态的容器。 | 4. 全局 Shell 区域规范 | Shell / 状态 | SHELL-005、U-330、U-420、U-521 | 中 | 确认 | 已合并为正式内容 |
| LA-008 | 用户端详情页采用主内容 + 辅助摘要双栏结构。 | 5. 页面模板库 | 模板 | TPL-002、U-310、U-420、U-520 | 中 | 确认 | 合并为正式内容 |
| LA-009 | 用户工作台为登录后默认首页。 | 6. Sitemap 到 Layout 映射总表 | 导航 / 路由 | U-200 | 中 | 确认 | 已合并为正式内容 |
| LA-010 | 用户端详情/交易流返回时保留来源页滚动位置和筛选条件。 | 7. 导航与路由规则 | 导航 / 状态 | U-310、U-320、U-330、U-420、U-520、U-521 | 中 | 确认 | 已合并为正式内容 |
| LA-011 | 用户端离线提示使用弱提示 banner，而不是全屏阻断。 | 8. 全局状态与边界 Layout | 状态 | GL-STATE-007 | 低 | 确认 | 已合并为正式内容 |

### 11.2 布局待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响页面 / Surface / Shell | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| LA-001 | 节点资料提交视图是否必须始终作为独立新页面，而不是在部分简单节点下回退为轻量抽屉？ | 1. 产品布局总览 | Shell / 模板 / 导航 | U-521、SF-001、TPL-006 | 中 | 已确认独立新页面 | 当前按独立新页面执行，如后续出现轻量节点再拆分新规则 |
| LQ-001 | 双域名用户端是否需要在顶栏中显式显示当前平台标识切换/徽标，还是仅通过品牌视觉区分，不额外显示平台标签？ | 1. 产品布局总览 | Shell / 导航 | SF-001、SHELL-001、U-100、U-300 | 中 | 确认 | 已按仅通过品牌视觉区分合并正文 |
...
```text
主 Layout 形态 改为 左侧导航 + 顶栏 + 主内容工作区 + 悬浮客服入口
```
