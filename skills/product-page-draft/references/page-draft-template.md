...
- 页面级假设 / 待确认编号规则：
  ...
  - 正文、表格、接口、状态、元素中出现的每个 `PA-*` / `PQ-*` 必须在文档末尾统一清单中出现。
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| ACT-001 | E-001 |  | 点击 / 输入 / 选择 / 拖拽 / 上传 / 长按 / 滑动 / 播放 / 暂停 |  |  | 本地状态更新 / API 调用 / 导航 / 打开弹窗 / 触发系统能力 |  |  |  |  |  |
...
> 本章节必须位于文档最后，用于用户统一确认。正文、表格、接口、状态中出现的所有 `PA-*` / `PQ-*` 必须在这里逐条列出。

### 10.1 页面假设清单

| 假设ID | 假设内容 | 首次出现位置 | 影响范围 | 影响元素 / Action / API | 置信度 | 用户确认状态 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PA-001 |  | 章节 / 表格行 / 元素ID / ActionID / API ID | 元素 / 状态 / 交互 / 接口 / 数据 / 样式 / 错误处理 |  | 高 / 中 / 低 | 待用户确认 / 已确认 / 已否定 / 已修改 | 确认为正式内容 / 删除 / 按用户修改替换 |

### 10.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 |  | 章节 / 表格行 / 元素ID / ActionID / API ID | 元素 / 状态 / 交互 / 接口 / 数据 / 样式 / 错误处理 |  | 高 / 中 / 低 | 待用户确认 / 已确认：... / 不需要 / 改为：... | 写入正式内容 / 删除 / 保留为风险 |
