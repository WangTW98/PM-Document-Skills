...
|---|---|---|---|---|---|---|---|
| PA-001 | 点击提交时，后台不但保存了目标岗位信息，同时也会触发真正的耗时 AI 分析任务（或将其推入队列），下个页面是查询或等待此分析结果。 | 7.2 接口清单 (API-001) | 接口及架构设计 | API-001, ACT-004 | 高 | 待确认 | 确认为正式内容 |

### 10.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | “薪资要求”是必填项还是选填项？这对 AI 简历分析的权重影响大吗？ | 4 页面元素清单 (E-005) | 表单验证逻辑 | E-005 | 低 | 待确认 | 暂按选填处理 |
