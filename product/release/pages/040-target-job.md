...
| E-002 | 目标岗位输入 | input | S-002 | 是 | 总是展示 | 如：产品经理 | 标准输入框 | target_job_title |
| E-003 | 行业选择器 | select | S-002 | 是 | 总是展示 | 请选择行业 | 底部弹出层/新页面 | target_industry |
| E-004 | 期望城市 | input | S-002 | 是 | 总是展示 | 请选择城市 | 选择器 | target_city |
| E-005 | JD 职位描述 | textarea | S-002 | 否 | 总是展示 | 粘贴目标岗位的 JD 信息或补充特殊要求 | 高度 120px | extra_info |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| ACT-001 | E-006 | 提交岗位目标 | 点击 | 表单校验通过 | 必填项不为空 | 调 API-002 | 进入分析页 | 提示提交失败 | 跳转 U-050 | EVT-003 | 生成分析点击 |
| ACT-002 | E-003 | 选择行业 | 点击 | 无 | 无 | 打开选择器 | 选中并回填 | 无 | 状态更新 | EVT-004 | 行业选择点击 |
...
| EVT-003 | 生成分析点击 | click | ACT-001 | 点击主按钮 | 每次触发计 1 次 | actionId | 实时 | target_job, target_industry | | 核心转化点 |
| EVT-004 | 行业选择点击 | click | ACT-002 | 点击行业选择 | 每次触发计 1 次 | actionId | 实时 | selected_industry | | |
...
|---|---|---|---|---|---|---|---|---|
| API-002 | 提交分析任务 | POST | /api/analysis/submit | ACT-001 | {resume_id, target_job, extra_info} | {task_id} | 跳转详情 | 错误提示 |
