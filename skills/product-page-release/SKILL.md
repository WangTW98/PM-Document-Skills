...
description: Process exactly one page draft Markdown document per invocation from product/development/pages. By default, when the user has not explicitly asked for a final/release/正式版 page, apply completed PA-/PQ- decisions and 用户补充描述 edits into a new versioned draft while preserving the page assumption, confirmation, and empty supplement workflow for another review loop. Only when the user explicitly asks for final/release output, generate the confirmed page document under product/release/pages by removing PA-/PQ- references, assumptions, open questions, draft-only notes, supplement sections, and uncertain language. Never process multiple page files in one execution.
---
...
Process exactly one page MD from `product/development/pages`.

This skill has two modes:

- Draft revision mode, which is the default unless the user explicitly asks for a final/release/正式版 page.
- Final release mode, which keeps the existing release behavior and writes the confirmed page source under `product/release/pages`.

In draft revision mode, apply the user's completed edits from the final page assumption/question list and `用户补充描述`, then write a new versioned draft that still contains the page assumption/question list and an empty `用户补充描述` section for the next review loop. In final release mode, the release page is confirmed source material for downstream design, UI, API, and implementation work, so it must not contain unresolved assumptions, open questions, `PA-*` / `PQ-*` IDs, the draft `用户补充描述` section, or wording that asks the user to confirm page details.
...
- A single draft page file under `product/development/pages/*.md`, including a versioned draft such as `product/development/pages/010-login-v2.md` when continuing a review loop.
...
- A versioned draft beside the source page draft, using the next available suffix, for example `product/development/pages/010-login-v2.md`, `product/development/pages/010-login-v3.md`.
...
- Input: `product/development/pages/010-login-v3.md`
- Output: `product/release/pages/010-login.md`
...
0. Determine mode before writing any file.
   - Use final release mode only when the user explicitly asks to generate the final/release/正式版 page, publish the confirmed release, write under `product/release/pages`, or remove all draft confirmation structures.
   ...
   - If the request is ambiguous, choose draft revision mode. Do not create or overwrite `product/release/pages` output without explicit final-release intent.
...
1. Select one source page.
   ...
   - Treat the selected latest file as the source draft for both draft revision and final release mode.
...
3. Validate according to the selected mode.
   - Analytics content is release-critical. Missing `## 7. 埋点事件统计设计`, missing `埋点事件ID`, or missing `EVT-*` must block release.
   - In draft revision mode, unresolved `PA-*` / `PQ-*` items are allowed and should remain in the revised draft if still material.
   - In draft revision mode, completed decisions and concrete supplement edits must be applied; unresolved or newly exposed uncertainty must be represented as page assumptions/questions in the revised draft, not as blockers unless the page cannot be updated coherently.
   - In draft revision mode, ambiguous or contradictory supplement items should become new `PQ-*` confirmation rows when a reasonable draft can still be produced.
...
4. Analyze and apply user supplement.
   - Treat non-empty `用户补充描述` content as user-confirmed modification instructions for this page.
   ...
   - In final release mode, do not carry the `用户补充描述` section or its raw notes into the release file. Its content must be incorporated as confirmed requirements.
...
6. Rewrite the page as release content when in final release mode.
...
7. Rewrite as a new review draft when in draft revision mode.
   ...
   - Save beside the source page draft using the next available version suffix. If the source is `010-login.md`, write `010-login-v2.md`; if the source is already `010-login-v2.md`, write `010-login-v3.md`.
   - Do not write any `product/release/pages` output in draft revision mode.

8. Save and verify.
   - In final release mode, ensure `product/release/pages` exists.
   - In final release mode, preserve the input page filename relative to `product/development/pages`, but strip a trailing draft revision suffix like `-v2` or `-v3` from the release filename.
   - In final release mode, write the release page under `product/release/pages`.
   - In final release mode, run `references/page-release-quality-checklist.md` manually before finishing.
   - In draft revision mode, verify the revised draft still has the unified page assumption/question list, mandatory analytics section, `EVT-*` IDs, and an empty final `用户补充描述` section.
