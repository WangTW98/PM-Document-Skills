...
- 来源主稿：`product/development/pages/060-service-detail/index.md`
- 来源 Sitemap 行：
  - 生成顺序：60
  - 页面ID：U-310
  - 父页面ID：U-300
...
- 当前输出文件：product/development/pages/060-service-detail/index-v2.md
- 页面级假设 / 待确认编号规则：
...
| 出口 | 下单确认 (U-320) | 点击“立即下单” | 商品 ID、选定 SKU ID、购买数量 | 需登录，未登录跳转 U-100 |
...
| quantity | number | local state | 1 | 价格计算 | 购买数量 |
...
| S-003 | 固定操作底栏 | footer | 总是显示 | E-005 | 移动端/窄屏底部固定 | - |  |
...
| E-003 | SKU 选择器 | radio-group | S-001 | 是 | 总是显示 | 规格名称 | 按钮块状平铺样式 | skus |  |
| E-004 | 数量选择器 | stepper | S-001 | 是 | 总是显示 | 1 | - | quantity |  |
...
| ACT-004 | E-004 | 修改数量 | 点击 +/- | - | 1 <= q <= 99 | 更新购买数量 | 总价重算 (如显示) | - | - | - | - | 数量微调不埋点 |  |
...
| EVT-003 | 立即下单点击 | click | E-005 | 点击 | 每次触发 1 次 | actionId | 实时 | product_id, sku_id, is_logged_in, quantity | result | 支付转化 |  |
