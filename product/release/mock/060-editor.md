...
> 输出路径：`product/release/mock/060-editor.md`。本文档只描述页面展示内容与 mock 数据，不描述交互执行、埋点逻辑、接口调用或业务执行逻辑。
...
- 当前输出文件：`product/release/mock/060-editor.md`
- 页面名称：简历编辑器
...
|---|---|---|---|---|---|---|---|---|---|---|
| STATE-001 | E-005 | 呼吸态 | 加载 | 检测到内容可优化时 | 发现 [N] 处可优化项 | 悬浮球带有微光边缘 | 缩放动画 | 浮窗提示 | 静态 | |
...
| AM-002 | ACT-003 | E-006 | 采纳 AI 建议 | 原文案被新文案替换 | 已采纳 | 采纳失败 | Toast: 内容已更新 | 静态 | |
...
|---|---|---|---|
| Tab 导航 | 基本信息、工作经历、项目经历... | | |
| 表单 Label | 姓名、电话、邮箱、公司名、职责描述 | | |
| 简历详情 | | [张三, 138..., 工作描述] | 接口返回 |
| AI 建议 | | [原文, 优化建议文案] | 接口返回 |
| 保存状态 | 已自动保存 | 正在保存... | 系统状态 |
