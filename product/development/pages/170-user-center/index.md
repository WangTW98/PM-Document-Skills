...
- 来源 Sitemap 行：
  ...
  - 页面名称：消息与个人中心
  - 页面类型：首页
  - Sitemap 页面级MD文件：product/development/pages/170-user-center.md
- 当前输出文件：product/development/pages/170-user-center/index.md
...
- 用户目标：查看个人基本资料、处理未读消息，并管理账号安全（如修改密码）。
...
| E-002 | 消息中心入口 | button | S-002 | 是 | 总是显示 | 消息中心 | 包含红点 | unread_count |  |
| E-003 | 账号设置入口 | button | S-002 | 是 | 总是显示 | 账号设置 | - | - |  |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 如果用户涉及多公司切换，将在该页提供一个“切换当前公司”的独立入口或在账号设置中管理 | 2. 页面结构 | 交互 | - | 中 | 确认 | 公司不是账户体系下的字段，不需要切换，当前页面也不需要展示公司相关内容 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否需要在个人中心展示“已完成案件”的总数或勋章？当前 MVP 仅展示基础信息。 | 1.1 页面目标 | UI 元素 | - | 低 | 确认 | 不需要 |

## 12. 用户补充描述

```text
1，用户支持设置接受的消息通知类型：站内通知、邮件、短信；
2，用户可添加多个手机号作为消息接收渠道，每个手机号可设置接收消息的接收方式；
```
