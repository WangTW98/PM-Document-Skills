...
- 当前输出文件：product/development/pages/030-user-todo-list/index.md
...
| E-007 | 去处理按钮 | button | S-003 | 是 | 列表项内 | 去处理 | 品牌色，描边或实色 | - |  |
...
| E-006 | 已过期 | 当前时间 > 截止时间 | item.deadline_passed | 置灰背景 | 点击查看详情 | - | 已过期 | - | PA-001 |
| E-007 | 启用 | 待办未处理 | item.status="pending" | - | 跳转处理页 | - | - | - |  |
...
| ACT-002 | E-007 | 点击处理 | 点击 | - | - | 页面跳转 | - | - | 根据类型跳转 (U-520/U-330/U-620) | EVT-003 | 去处理点击 | - |  |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 对于已过期的待办，依然允许点击“去处理”，但在样式上通过置灰或过期标签进行区分 | 5. 元素状态矩阵 | 样式、逻辑 | E-006 | 中 | 确认 | 已过期待办不允许处理 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | “忽略待办”后是否支持在“已忽略”筛选下恢复？ | 交互矩阵 | 逻辑 | API-002 | 低 | 确认 | 不支持 |
