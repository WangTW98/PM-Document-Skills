...
> 使用方式：按本模板的标题层级、章节顺序和核心表格结构生成 `product/development/product-sitemap-draft.md`。
> 不得重命名、删除、合并 `## 0` 到 `## 6` 顶级章节；如某项信息不足，也必须保留章节并用带 ID 的 `假设 A-xxx` 或 `待确认 Q-xxx` 标记。
> 不确定内容必须在正文中显式标记，并在 `## 5. 假设与待确认统一清单` 中逐条汇总。
> 文档末尾必须保留 `## 6. 用户补充描述`，供用户填写自然语言修改。
