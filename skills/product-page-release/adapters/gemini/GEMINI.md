...
5. Apply every release handling decision from the final `页面假设与待确认统一清单`.
6. Write the release page to `product/release/pages` using the same relative filename.
7. Ensure the release document contains no `PA-*`, `PQ-*`, assumptions, open questions, or confirmation workflow sections.
