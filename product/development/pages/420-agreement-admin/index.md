...
- 来源 Sitemap 行：
  ...
  - 页面名称：协议管理
  - 页面类型：列表与版本控制
  - Sitemap 页面级MD文件：product/development/pages/420-agreement-admin.md
- 当前输出文件：product/development/pages/420-agreement-admin/index.md
...
| S-003 | 协议正文编辑器 | modal/page | 点击新建/编辑时 | E-004, E-005 | 富文本编辑器 + 配置项 | - | PA-001 |
...
| E-004 | 富文本编辑器 | other | S-003 | 是 | 总是显示 | [Quill/Editor 占位] | - | agreement.content | PA-001 |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 协议编辑器支持基础 HTML 富文本（加粗、列表、链接、表格），不支持嵌入脚本或外部 CSS。 | 2. 页面结构 | 编辑能力 | E-004 | 高 | 确认 | 发布到正式 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 当协议版本更新时，是否需要强制弹窗让所有已登录用户重新勾选“同意”？当前假设为仅在下一次敏感操作（如支付）时检查。 | 1.1 页面目标 | 业务流程 | - | 中 | 确认 | 不需要 |
