...
| 入口 | 拦截器 (Interceptor) | Token 过期或未登录用户触发任何需登录功能时 | redirect_url (拦截前页面) | 统一的登录拦截器机制 |
...
| S-004 | 第三方登录区 | footer | 常驻 | E-008, E-009 | 页面底部，小字“其他方式登录”+微信Icon | |
...
| E-004 | 验证码输入框 | input | S-002 | 是 | 常驻 | "请输入验证码" (placeholder) | 仅数字键盘，限制6位 | `verifyCode` |
...
| E-008 | 微信快捷登录按钮 | button | S-004 | 否 | 常驻 | 微信图标 | 圆形图标按钮或胶囊按钮 | |
| E-009 | "其他登录方式"提示 | text | S-004 | 否 | 常驻 | "或使用以下方式登录" | 灰色小字，带左右线条装饰 | |
...
| E-006 | 禁用 | `phone` 或 `verifyCode` 为空时 | 表单校验 | 按钮置灰降低透明度 | | 点击 | | 表单填满且有效 |
| E-006 | 加载 | 接口请求中 | 接口请求 | 按钮显示 Loading 图标 | | 重复点击 | | 请求返回结果 |
| E-007 | 提示 | 点击登录/微信但未勾选时 | `!isAgreed` | 触发勾选框抖动动画 | | | Toast: "请先阅读并同意用户协议" | 用户主动勾选 |

## 6. 交互 Action 与执行效果

| ActionID | 触发元素ID | 交互名称 | 用户动作 | 前置条件 | 校验规则 | 执行动作 | 成功结果 | 失败结果 | 页面跳转 / 状态变化 | 埋点事件 |
|---|---|---|---|---|---|---|---|---|---|---|
...
| ACT-003 | E-008 | 微信登录 | 点击 | 无 | 必须勾选 E-007 | 调用微信 SDK + API (API-003) | 保存 Token | Toast 授权取消/失败 | 成功跳首页/来源页 | `click_login_wechat` |
...
| API-003 | 微信授权登录 | POST | `/api/v1/auth/login/wechat` | ACT-003 | `{ "wechat_code": "string" }` | `{ "token": "string", "user_info": {}, "is_new_user": boolean }` | 写入本地缓存，页面跳转 | Toast 授权失败 |

**注**：首次调用 API-003 微信登录时，后端将根据微信API返回的用户手机号自动注册用户，不强制跳转绑定手机号页面。
...
| EDGE-004 | 微信客户端异常 | 拉起微信授权失败或未安装微信 | 隐藏或提示无法拉起 | 微信授权请求失败 | 使用手机号登录 | 取消微信调用 | "未检测到微信客户端或拉起失败，请使用手机号登录" |
