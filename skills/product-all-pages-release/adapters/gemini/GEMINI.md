...
9. For each selected page, follow `skills/product-page-release/SKILL.md` in the selected mode and process exactly one page before updating status, including analysis and application of non-empty `用户补充描述`; in draft revision mode, reject version-only copies.
