...
- 当前输出文件：product/development/pages/160-file-downloads/index.md
...
| S-003 | 批量操作区 | footer | 选中文件时显示 | E-005 | 底部悬浮/侧边条 | - | PA-001 |
...
| E-004 | 下载图标按钮 | button | S-002 | 是 | 总是显示 | 下载图标 | 蓝色 | - |  |
| E-005 | 批量下载按钮 | button | S-003 | 是 | 选中文件 > 0 | 批量下载 ({n}) | 品牌色实色 | - | PA-001 |

## 5. 元素状态矩阵

| 元素ID | 状态 | 触发条件 | 状态来源 | 样式定义 | 可执行动作 | 禁止动作 | 提示文案 / 反馈 | 恢复条件 | 相关 PA/PQ |
|---|---|---|---|---|---|---|---|---|---|
| E-003 | 多选选中 | 点击行首 checkbox | row.selected | 背景色淡蓝 | - | - | - | - |  |
| E-005 | 显隐状态 | 选中数量变更 | selected_count | display: block/none | 点击打包下载 | - | 正在打包... | - | PA-001 |
...
| ACT-002 | E-004 | 单个下载 | 点击图标 | - | - | 触发下载 | 文件开始下载 | 报错 | - | EVT-003 | 单文件下载 | - |  |
| ACT-003 | E-005 | 批量下载 | 点击 | 选中 > 0 | - | 请求后端打包并下载 | 弹出 ZIP 下载 | 报错 | - | EVT-004 | 批量下载操作 | - | PA-001 |
...
| API-002 | 批量打包文件 | POST | /api/service/batch-download | ACT-003 | {file_ids[]} | {zip_url} | 触发 ZIP 下载 | 提示打包失败 | PA-001 |
...
|---|---|---|---|---|---|---|---|
| PA-001 | 批量下载功能通过后端异步打包为 ZIP 后通过临时链接下发，前端需处理“打包中”的加载态 | 6. 交互 Action | 批量下载 | E-005, API-002 | 中 | 确认 | 不需要批量下载功能，所有文件均单独点击下载按钮 |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 官方文件是否包含“预览”功能（如直接在浏览器查看 PDF）？当前假设仅支持下载。 | 4. 页面元素清单 | 交互模式 | - | 低 | 确认 | 仅支持下载，不支持预览 |
