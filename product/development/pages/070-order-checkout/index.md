...
|---|---|---|---|---|---|---|---|
| PA-001 | 下单页的动态字段由 SKU 绑定的服务模板决定，后端接口实时下发 schema 用于渲染 | 1.4 全局页面属性 | 动态表单 | E-004, API-001 | 高 | 待用户确认 |  |
| PA-002 | 如果用户之前有过下单或在“账号设置”中维护过公司信息，进入该页时将自动填入最近使用的信息 | 6. 交互 Action | 表单填充 | ACT-002 | 中 | 待用户确认 |  |

### 11.2 页面待确认清单

| 待确认ID | 待确认问题 | 首次出现位置 | 影响范围 | 影响元素 / Action / API / 埋点事件 | 优先级 | 用户确认结果 | Release 处理方式 |
|---|---|---|---|---|---|---|---|
| PQ-001 | 是否支持在下单页“更换公司”或“新增公司”？当前假设为直接在表单项内编辑。 | 3. 页面布局 | 交互逻辑 | E-002 | 中 | 待用户确认 |  |
