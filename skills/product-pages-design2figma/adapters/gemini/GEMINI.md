...
13. Then create only the user-selected release pages in Figma, and for each page use only its matched layout family. Do not mix layout families within one page.
14. Name every created page frame as `<sitemap 页面级MD文件 basename>-<页面ID>-<页面标题>`.
15. Do not use `product/release/design/*.md` as the primary source.
16. Do not process the full sitemap in this skill; use `product-all-pages-design2figma` for that.
17. Do not leave unresolved hierarchy, overflow, clipping, or layer-order issues in the created Figma nodes.
